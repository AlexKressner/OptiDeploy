...
async def get_solution_by_solution_id(
    solution_id: str, db
) -> Union[SolutionSchema, None]:
...
async def get_solutions_by_instance_id(
    instance_id: str, db
) -> Union[List[SolutionSchema], None]:
...
async def delete(
    solution_id: str, db: AsyncIOMotorDatabase
) -> Union[DeleteResponseSchema, None]:
    solution = await db["solution_collection"].find_one_and_delete(
        {"_id": ObjectId(solution_id)}, projection=["_id"]
    )
    if solution:
